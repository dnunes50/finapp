...
const CORES  = ['#1D9E75','#378ADD','#EF9F27','#7F77DD','#D85A30','#D4537E','#888780','#185FA5','#639922','#BA7517']
...
export default function ConfigContas() {
  ...
  const [form, setForm]       = useState({ nome:'', tipo:'corrente', banco:'', saldo_inicial:'0', cor:'#1D9E75', incluir_patrimonio:true })
  ...
  async function salvar(e) {
    ...
    setModal(false)
    setForm({ nome:'', tipo:'corrente', banco:'', saldo_inicial:'0', cor:'#1D9E75', incluir_patrimonio:true })
    setSaving(false)
    load()
  }
    ...
    <AppLayout>
      <div style={{ padding:'24px', maxWidth:'700px' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px' }}>
          <div>
            <h1 style={{ fontSize:'20px', fontWeight:'600', color:'#1A1A18' }}>Contas bancárias</h1>
            <p style={{ fontSize:'13px', color:'#888' }}>{contas.length} contas cadastradas</p>
          </div>
            ...
            contas.map((c,i) => (
              <div key={c.id} style={{ display:'flex', alignItems:'center', gap:'12px', padding:'12px 0', borderBottom: i<contas.length-1?'1px solid #F0F0EE':'none', opacity:c.ativo?1:0.5 }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:'11px', color:'#AAA' }}>{tipoLabel[c.tipo]||c.tipo} · Saldo inicial: R$ {fmt(c.saldo_inicial)}</div>
                </div>
                <div style={{ textAlign:'right' }}>
                  <div style={{ fontSize:'13px', fontWeight:'500', color:'#1A1A18' }}>{c.banco||'—'}</div>
                  {c.incluir_patrimonio && <div style={{ fontSize:'10px', color:'#1D9E75' }}>No patrimônio</div>}
                </div>
                <button onClick={()=>toggleAtivo(c.id,c.ativo)} style={{ padding:'4px 10px', fontSize:'10px', border:'1px solid #E8E8E5', borderRadius:'6px', cursor:'pointer', background:'#FFF', color:'#666' }}>
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.4)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}>
          <div style={{ background:'#FFF', borderRadius:'16px', padding:'24px', width:'100%', maxWidth:'400px', margin:'16px' }}>
            ...
            <form onSubmit={salvar}>
              ...
              <div style={{ marginBottom:'12px' }}>
                <label style={{ display:'block', fontSize:'11px', color:'#888', marginBottom:'4px' }}>Banco</label>
              ...
              <div style={{ marginBottom:'16px', display:'flex', alignItems:'center', gap:'8px' }}>
                ...
                <label htmlFor="patrimonio" style={{ fontSize:'12px', color:'#555' }}>Incluir no patrimônio total</label>
              </div>
