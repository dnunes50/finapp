...
export default function Metas() {
  ...
  const [form, setForm]     = useState({ nome:'', descricao:'', valor_alvo:'', valor_atual:'0', prazo:'', cor:'#1D9E75' })
  ...
  async function salvar(e) {
    ...
    setModal(false)
    setForm({ nome:'', descricao:'', valor_alvo:'', valor_atual:'0', prazo:'', cor:'#1D9E75' })
    setSaving(false)
    load()
  }
    ...
    <AppLayout>
      <div style={{ padding:'24px', maxWidth:'900px' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px' }}>
          <div>
            <h1 style={{ fontSize:'20px', fontWeight:'600', color:'#1A1A18' }}>Metas financeiras</h1>
            <p style={{ fontSize:'13px', color:'#888' }}>{metas.filter(m=>m.status==='ativa').length} metas ativas</p>
          </div>
        ...
        {loading ? <p style={{ color:'#AAA', fontSize:'13px' }}>Carregando...</p> :
          ...
          metas.map(m => {
            ...
            return (
              <div key={m.id} style={{ background:'#FFF', border:'1px solid #E8E8E5', borderRadius:'12px', padding:'16px', marginBottom:'10px' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'10px' }}>
                  <div>
                    <div style={{ fontSize:'14px', fontWeight:'500', color:'#1A1A18', marginBottom:'2px' }}>{m.nome}</div>
                    {m.prazo && <div style={{ fontSize:'11px', color:'#AAA' }}>Prazo: {new Date(m.prazo+'T12:00:00').toLocaleDateString('pt-BR')}</div>}
                  </div>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'6px' }}>
                  <div style={{ flex:1, height:'6px', background:'#F0F0EE', borderRadius:'3px', overflow:'hidden' }}>
                    <div style={{ height:'100%', width:`${pct}%`, background:m.cor||'#1D9E75', borderRadius:'3px', transition:'width 0.5s' }}></div>
                  </div>
                  <span style={{ fontSize:'12px', fontWeight:'500', color: m.cor||'#1D9E75', width:'36px', textAlign:'right' }}>{pct.toFixed(0)}%</span>
                </div>
                ...
                {m.status === 'ativa' && (
                  <div style={{ display:'flex', gap:'6px', marginTop:'10px' }}>
                    <button onClick={()=>alterarStatus(m.id,'pausada')} style={{ padding:'4px 10px', fontSize:'10px', border:'1px solid #E8E8E5', borderRadius:'6px', cursor:'pointer', background:'#FFF', color:'#666' }}>Pausar</button>
      ...
      {modal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.4)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}>
          <div style={{ background:'#FFF', borderRadius:'16px', padding:'24px', width:'100%', maxWidth:'400px', margin:'16px' }}>
            ...
            <form onSubmit={salvar}>
              ...
              <div style={{ marginBottom:'16px' }}>
                <label style={{ display:'block', fontSize:'11px', color:'#888', marginBottom:'4px' }}>Cor</label>
                <div style={{ display:'flex', gap:'8px' }}>
                  {['#1D9E75','#378ADD','#EF9F27','#7F77DD','#D85A30','#D4537E'].map(c=>(
