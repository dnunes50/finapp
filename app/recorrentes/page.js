    ...
    <AppLayout>
      <div style={{ padding:'24px', maxWidth:'800px' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px' }}>
          <div>
            <h1 style={{ fontSize:'20px', fontWeight:'600', color:'#1A1A18' }}>Recorrentes</h1>
            <p style={{ fontSize:'13px', color:'#888' }}>{recorrentes.filter(r=>r.ativo).length} ativos</p>
          </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px', marginBottom:'20px' }}>
          <div style={{ background:'#FFF', border:'1px solid #E8E8E5', borderRadius:'12px', padding:'12px 16px' }}>
            <div style={{ fontSize:'11px', color:'#888', marginBottom:'3px' }}>Despesas/mês</div>
            <div style={{ fontSize:'16px', fontWeight:'600', color:'#A32D2D' }}>-R$ {fmt(totalMensal)}</div>
          </div>
          <div style={{ background:'#FFF', border:'1px solid #E8E8E5', borderRadius:'12px', padding:'12px 16px' }}>
            <div style={{ fontSize:'11px', color:'#888', marginBottom:'3px' }}>Receitas/mês</div>
            <div style={{ fontSize:'16px', fontWeight:'600', color:'#0F6E56' }}>+R$ {fmt(totalReceitas)}</div>
          </div>
        </div>
            ...
            recorrentes.map((r,i) => (
              <div key={r.id} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 0', borderBottom: i<recorrentes.length-1?'1px solid #F0F0EE':'none', opacity: r.ativo?1:0.5 }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ fontSize:'10px', color:'#AAA' }}>Todo dia {r.dia_vencimento} · {r.contas?.nome||'—'} · {r.categorias?.nome||'—'}</div>
                </div>
                <span style={{ fontSize:'13px', fontWeight:'500', color: r.tipo==='receita'?'#0F6E56':'#1A1A18', flexShrink:0 }}>
                  {r.tipo==='receita'?'+':'-'}R$ {fmt(r.valor)}
                </span>
                <button onClick={()=>toggleAtivo(r.id,r.ativo)} style={{ padding:'4px 10px', fontSize:'10px', border:'1px solid #E8E8E5', borderRadius:'6px', cursor:'pointer', background: r.ativo?'#FFF':'#F0F0EE', color:'#666' }}>
